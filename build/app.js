...
/***/ function(module, exports, __webpack_require__) {
	...
	d3.queue().defer(d3.json, 'data/dogs.json').defer(d3.json, 'data/cats.json').defer(d3.json, 'data/data.json').await(display);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function createHist() {
	  ...
	  var xScale = d3.scaleLinear().rangeRound([0, width]).domain([0, 25]);
	  ...
	  function setupScales() {
	    ...
	    data.ids.forEach(function (id) {
	      xMins.push(d3.min(data[id].hist, function (d) {
	        return d.x1;
	      }));
	      xMaxs.push(d3.max(data[id].hist, function (d) {
	        return d.x1;
	      }));
	      yMins.push(d3.min(data[id].hist, function (d) {
	        return d.freq;
	      }));
	      yMaxs.push(d3.max(data[id].hist, function (d) {
	        return d.freq;
	      }));
	    });
	  ...
	  function update() {
	    data.ids.forEach(function (id) {
	      ...
	      var bar = idG.selectAll('.bar').data(data[id].hist).enter().append('g').classed('bar', true).classed(id, true).attr('transform', function (d) {
